...
var letterCombinations = function (digits) {
    ...
    let res = [];
    const backtrack = (i, s) => {    // s is current string with which the digit at index i will form combinations, digit at 0 = 2 , map[2] = abc will form combination with '' => [a, b ,c]
        if (i == digits.length) {// ex- when i = 2
            res.push(s);
            return;
        }

        let cur = map[digits[i]];   // 'abc' for 2, 'def' for 3, when i = 0 , map["2"] = 'abc' ==> cur 
